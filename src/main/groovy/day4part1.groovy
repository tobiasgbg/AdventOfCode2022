...
static void main(String[] args) {
    ...
    file.eachLine {if (CampCleanup.fullyContains(it)){result++}}

    println("Sum ${result}")
}

class CampCleanup {
    static def fullyContains(String a) {
        return containsOther(a.split(",")[0],a.split(",")[1])
                || containsOther(a.split(",")[1],a.split(",")[0])
    }

    static def fullyContains(String a, String b) {
    ...
    static def isIntersecting(String a) {
        return intersectsOther(a.split(",")[0],a.split(",")[1])
    }

    static def intersectsOther(String a, String b) {
        return (a.split("-")[1] as Integer) >= (b.split("-")[0] as Integer) &&
                (a.split("-")[0] as Integer) <= (b.split("-")[1] as Integer)
    }
}